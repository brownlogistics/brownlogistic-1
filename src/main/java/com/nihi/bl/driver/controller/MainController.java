...
public class MainController {
	...
	public DashboardResponse getDashboardDetails(@RequestBody DashboardRequest request){
		...
		try {
			...
			//System.out.println("Inside getDashboardDetails method : request : "+request.toString());
			...
			List<DriverTripWeekInfoEntity> list=driverCurrentWkRepository.getDetailByDriverId(Integer.parseInt(request.getDriverId()));
			if(list.size()>0){
	...
	public DetailsResponse getDetails(@RequestBody DetailsRequest request){
		...
		try {
			...
			//System.out.println("Inside getDetails method : request : "+request.toString());
			if(request.getFrieghtId()==null){
				res.setStatusCode("0002");
				res.setStatusMessage("Frieght Id can't be blank");
				return res;
			}
			List<DriverTripMonthInfoEntity> list=driverTripMonthRepository.findByBillNo(String.valueOf(request.getFrieghtId()));
